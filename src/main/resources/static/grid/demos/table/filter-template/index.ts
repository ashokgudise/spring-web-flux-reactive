...
import { CheckBox, Table } from "../../../source/typescript/smart.elements"
...
window.onload = function () {
    ...
    document.getElementById('filterCheckboxesContainer')!.addEventListener('change', function () {
        ...
        if (!maleChecked) {
            const filterObject1: object = filterGroup.createFilter('string', 'male', 'NOT_EQUAL');

            filterGroup.addFilter('and', filterObject1);
        }

        if (!femaleChecked) {
            const filterObject2: object = filterGroup.createFilter('string', 'female', 'NOT_EQUAL');
