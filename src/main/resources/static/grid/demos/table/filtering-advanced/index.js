...
window.Smart('#table', class {
    get properties() {
        return {
            ...
            columns: [
                {
                    label: 'Name', dataField: 'name', dataType: 'string', width: 275, formatFunction(settings) {
                ...
                {
                    label: '', dataField: 'action', dataType: 'string', allowSort: false, formatFunction(settings) {
...
function createCustomFilterRow(columnLabel) {
    const row = document.createElement('div'), conditionInput = document.createElement('smart-input'), valueInput = document.createElement('smart-input'), confirmButton = document.createElement('smart-button');
...
window.onload = function () {
    const table = document.getElementById('table'), numberOfAppliedFilters = document.getElementById('numberOfAppliedFilters'), filterIcon = document.getElementById('filterIcon'), filterRowsContainer = document.getElementById('filterRowsContainer'), filterMenu = document.getElementById('filterMenu'), removeMenu = document.getElementById('removeMenu');
    let rowToRemove;
    ...
    filterRowsContainer.addEventListener('click', function (event) {
        ...
        if (confirmButton) {
            const filterRow = confirmButton.parentElement, conditionInput = filterRow.firstElementChild, valueInput = filterRow.children[1], dataField = confirmButton.dataField, condition = conditionInput.$.input.dataValue, value = valueInput.value, menuItem = filterMenu.querySelector(`smart-menu-item[value="${dataField}"]`);
            if (!condition || !value) {
                ...
                const filterGroup = new window.Smart.Utilities.FilterGroup(), filterObject = filterGroup.createFilter('string', value, condition);
