...
const shareData = [
    ...
    numberRenderer = new window.Smart.Utilities.NumberRenderer();
    ...
    get properties() {
        return {
            ...
            columns: [
                ...
                { label: 'Price', dataField: 'price', dataType: 'number', formatFunction(settings: formatFunctionSettings) { settings.template = numberRenderer.formatNumber(settings.value, 'C2'); }, responsivePriority: 1 },
                {
                    label: 'Change', dataField: 'change', dataType: 'number', formatFunction(settings: formatFunctionSettings) {
                        settings.template = (settings.value > 0 ? '+' : '') + numberRenderer.formatNumber(settings.value, 'F2');
                    },
                    responsivePriority: 2
                },
                {
                    label: 'Change %', dataField: 'changePercentage', dataType: 'number', formatFunction(settings: formatFunctionSettings) {
                        settings.template = (settings.value > 0 ? '+' : '') + numberRenderer.formatNumber(settings.value, 'F2') + '%';
                    },
                    responsivePriority: 1
                },
                { label: 'Open', dataField: 'open', dataType: 'number', formatFunction(settings: formatFunctionSettings) { settings.template = numberRenderer.formatNumber(settings.value, 'C2'); }, responsivePriority: 1 },
                { label: 'High', dataField: 'high', dataType: 'number', formatFunction(settings: formatFunctionSettings) { settings.template = numberRenderer.formatNumber(settings.value, 'C2'); }, responsivePriority: 2 },
                { label: 'Low', dataField: 'low', dataType: 'number', formatFunction(settings: formatFunctionSettings) { settings.template = numberRenderer.formatNumber(settings.value, 'C2'); }, responsivePriority: 2 },
                { label: 'Volume', dataField: 'volume', dataType: 'number', formatFunction(settings: formatFunctionSettings) { settings.template = numberRenderer.formatNumber(settings.value, 'N0'); }, responsivePriority: 1 }
            ]
        };
    }
});
