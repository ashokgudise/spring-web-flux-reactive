    ...
    get properties() {
        return {
            ...
            columns: [
                ...
                {
                    label: '', dataField: '', width: 50, allowEdit: false, formatFunction(settings: { value: any, row: number | string, column: string, cell: HTMLTableCellElement, template?: string }) {
...
window.onload = function () {
    ...
    table.addEventListener('click', function (event) {
        const deleteIcon = (event.target as HTMLElement).closest('.delete-icon');
    ...
    (<Button>document.getElementById('addNewRow')).addEventListener('click', function () {
        table.rows.push({});
    });
};
